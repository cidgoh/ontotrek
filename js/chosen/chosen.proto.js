      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (this.include_option_in_results(option)) {
          ...
          if (!(option.group && !this.group_search)) {
            ...
            if (synonymEl==null) var search_synonyms = false
            else var search_synonyms = this.search_string_match(synonymEl.value, regex);

            if ((option.search_match || search_synonyms ) && !option.group) {
              results += 1;
            }
            if (option.search_match || search_synonyms) {
              if (search_synonyms) option.search_text += option.search_text + '<i>' + synonymEl.value + '</i>'
