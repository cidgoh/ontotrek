...
function get_node_radius(node, fancyLayout) {
  ...
  */
  return (node.radius > GRAPH_NODE_RADIUS) ? node.radius : GRAPH_NODE_RADIUS
}
...
function node_focus(node = {}) {
  ...
  */ 
  ...
  // Aim camera at node from z dimension
  // Unfortunately camera animations cause it to loose its "UP" position.  Solution?
  if (node.x) {
...
function set_directional_particles(){
  // NOT using force graph's directional particles right now
